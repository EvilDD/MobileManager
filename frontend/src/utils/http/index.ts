...
const generateSignature = () => {
  const rand = Math.floor(Math.random() * (99999999 - 10000000 + 1)) + 10000000;
  const ts = Math.floor(Date.now() / 1000);
  const dataToSign = `rand=${rand}&secretkey=${secretKey}&ts=${ts}`;
  const signature = CryptoJS.SHA256(dataToSign).toString();
  ...
  private httpInterceptorsRequest(): void {
    PureHttp.axiosInstance.interceptors.request.use(
      async (config: PureHttpRequestConfig): Promise<any> => {
        ...
        // 添加签名认证
        const { signature, rand, ts } = generateSignature();
        config.headers["X-SHA-Signature"] = signature;

        // 将rand和ts添加到URL参数中
        const url = new URL(config.url, window.location.origin);
        url.searchParams.set("rand", rand.toString());
